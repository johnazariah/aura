...
- **WorkflowService**: Full implementation with:
  ...
  - `AnalyzeAsync`: Runs issue-enricher agent with RAG context
...
- **External Templates**: `workflow-ENRICH.prompt`, `workflow-plan.prompt`, `step-execute.prompt`, `step-review.prompt`

### 4. RAG Integration

- Worktree auto-indexing on workflow creation
- RAG context included in analysis prompts
- issue-enricher agent template supports `{{#if context.RagContext}}`
