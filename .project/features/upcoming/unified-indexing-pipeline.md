...
1. **Two Complementary Pipelines**: RAG (file-level) + Code Graph (solution-level)
2. **Module Extension**: Modules can register specialized ingestors for RAG chunking
3. **Semantic Fidelity**: Code graph uses full Roslyn semantic model (requires solution context)
4. **Background Processing**: Both pipelines run asynchronously with progress tracking

## Two-Pass Indexing Architecture

Semantic code understanding requires **solution-level context** (project references, cross-file types, etc.). File-by-file processing loses this context. Therefore, we use two complementary passes:

### Pass 1: RAG Indexing (File-Level)
- **Entry Point:** `BackgroundIndexer` via `/api/index/background`
- **Granularity:** Per-file processing
- **Output:** Embedded chunks for semantic search
- **Speed:** Fast (no compilation needed)
- **Use Case:** "Find code similar to X", natural language queries

### Pass 2: Code Graph Indexing (Solution-Level)  
- **Entry Point:** `CodeGraphIndexer` via `/api/developer/codegraph/index`
- **Granularity:** Full solution/project analysis
- **Output:** Nodes and edges for structural queries
- **Speed:** Slower (requires Roslyn compilation)
- **Use Case:** "Find implementations of IFoo", "What calls this method?"

```text
┌─────────────────────────────────────────────────────────────────┐
│                     Extension UI                                 │
│   (calls both endpoints when indexing a codebase)               │
└─────────────────────────────────────────────────────────────────┘
         │                                    │
         ▼                                    ▼
┌─────────────────────────┐     ┌─────────────────────────────────┐
│  BackgroundIndexer      │     │  CodeGraphIndexer               │
│  (Foundation)           │     │  (Developer Module)              │
├─────────────────────────┤     ├─────────────────────────────────┤
│  Per-file processing    │     │  Solution-level Roslyn analysis  │
│  IngestorRegistry       │     │  Full semantic model             │
│  RAG chunks → pgvector  │     │  Graph nodes → PostgreSQL        │
└─────────────────────────┘     └─────────────────────────────────┘
```

## Content Processing Hierarchy (RAG Pass)

The RAG pass has **three** content processing approaches:

### 1. Native Ingestors (`IContentIngestor`)

**Purpose:** Precise, deterministic parsing using native language tooling

| Component | Parser | Produces | Use Case |
|-----------|--------|----------|----------|
| `CodeIngestor` | Regex-based | RAG chunks | Code files - fast structural chunking |
| `MarkdownIngestor` | Regex/string parsing | RAG chunks | Markdown files |
| `PlainTextIngestor` | Line splitting | RAG chunks | Fallback for unknown types |

**Characteristics:**

- **Deterministic** - same input always produces same output
- **Fast** - no LLM calls, native code execution
- **Precise** - exact line numbers, symbol resolution, type information
- **Specialized** - each ingestor handles specific file types

### 2. Agent-Based Ingestors (LLM-Powered)

**Purpose:** Flexible, intelligent parsing when native parsers aren't available

| Agent | Capability | Use Case |
|-------|------------|----------|
| `generic-code-ingester` | `ingest:*` | Fallback for any code file lacking native parser |

**Characteristics:**

- **Flexible** - can understand any language structure
- **Intelligent** - infers semantic meaning, understands intent
- **Non-deterministic** - output may vary between calls
- **Slower** - requires LLM API call
- **Costly** - consumes tokens

### 3. Processing Priority

`BackgroundIndexer` checks in this order:

```text
1. ICodeIngestor from IngestorRegistry
   ↓ (if matches file type)
   → Produces RAG chunks + Code graph nodes

2. IContentIngestor from IngestorRegistry
   ↓ (if matches file type)
   → Produces RAG chunks only

3. Agent with ingest:{extension} capability
   ↓ (if agent found)
   → LLM parses file, produces RAG chunks

4. PlainTextIngestor (fallback)
   → Splits file into chunks by lines
```
...
|----------|----------|-----|
| C# code | `RoslynCodeIngestor` | Semantic analysis, precise call graphs |
| TypeScript (future) | `TreeSitterIngestor` | Fast AST parsing |
| Obscure language | `generic-code-ingester` | LLM can understand structure |
| Markdown | `MarkdownIngestor` | Fast, deterministic |
| Plain text | `PlainTextIngestor` | Simple chunking |

### Module Extension Points

1. **Register Native Ingestor**: Module adds `ICodeIngestor` to `IngestorRegistry`

   ```csharp
   registry.Register(new RoslynCodeIngestor(...), priority: 100);
   ```

2. **Register Agent**: Module adds markdown agent to `agents/` directory
...
Native ingestors take priority because they're faster and more precise. Agents are the intelligent fallback.
