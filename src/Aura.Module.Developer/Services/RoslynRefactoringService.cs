...
public sealed class RoslynRefactoringService : IRoslynRefactoringService
{
    ...
    {
        _logger.LogInformation("Moving {Count} members from {Class} to partial file {Target}",
            request.MemberNames.Count, request.ClassName, request.TargetFileName);
        ...
        try
        {
            ...
            if (!typeSyntax.Modifiers.Any(m => m.IsKind(Microsoft.CodeAnalysis.CSharp.SyntaxKind.PartialKeyword)))
            {
                if (request.EnsureSourceIsPartial)
                {
                    ...
                    var partialToken = SyntaxFactory.Token(Microsoft.CodeAnalysis.CSharp.SyntaxKind.PartialKeyword)
                    ...
                    typeSyntax = (TypeDeclarationSyntax)newSourceRoot.DescendantNodes()
                        .First(n => n is TypeDeclarationSyntax t && t.Identifier.Text == request.ClassName);
                    sourceModified = true;
                }
                else
                {
                    return RefactoringResult.Failed(
                        $"Class '{request.ClassName}' is not partial. Set EnsureSourceIsPartial=true to add the modifier.");
                }
            }

            // Remove moved members from source
            var nodesToRemove = typeSyntax.Members
                .Where(m => membersToMove.Any(mtm => mtm.IsEquivalentTo(m)))
                .ToList();
            ...
            var targetTypeModifiers = SyntaxFactory.TokenList(
                typeSyntax.Modifiers.Where(m =>
                    ...
                    m.IsKind(Microsoft.CodeAnalysis.CSharp.SyntaxKind.SealedKeyword) ||
            ...
            {
                targetTypeModifiers = targetTypeModifiers.Add(
                    SyntaxFactory.Token(Microsoft.CodeAnalysis.CSharp.SyntaxKind.PartialKeyword)
                        .WithTrailingTrivia(SyntaxFactory.Space));
            }

            var targetTypeSyntax = SyntaxFactory.ClassDeclaration(typeSyntax.Identifier)
                .WithModifiers(targetTypeModifiers)
            ...
            var targetFileRoot = SyntaxFactory.CompilationUnit()
                ...
                .WithMembers(SyntaxFactory.SingletonList<MemberDeclarationSyntax>(
                    SyntaxFactory.FileScopedNamespaceDeclaration(SyntaxFactory.ParseName(namespaceName))
                        .WithMembers(SyntaxFactory.SingletonList<MemberDeclarationSyntax>(targetTypeSyntax))))
                .NormalizeWhitespace();
