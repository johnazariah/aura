...
public static class DeveloperEndpoints
{
        ...
        CancellationToken ct)
    {
        ...
        return Results.Ok(new
        {
            ...
            stories = stories.Select(w => new
            {
                ...
                automationMode = w.AutomationMode.ToString(),
                gitBranch = w.GitBranch,
                repositoryPath = w.RepositoryPath,
                worktreePath = w.WorktreePath,
                issueUrl = w.IssueUrl,
                issueProvider = w.IssueProvider?.ToString(),
                issueNumber = w.IssueNumber,
                issueOwner = w.IssueOwner,
                issueRepo = w.IssueRepo,
                currentWave = w.CurrentWave,
                waveCount = GetWaveCount(w),
                stepsDone = w.Steps.Count(s => s.Status == StepStatus.Completed),
                stepsTotal = w.Steps.Count,
                durationMinutes = Math.Round(((w.CompletedAt ?? DateTimeOffset.UtcNow) - w.CreatedAt).TotalMinutes, 1),
        ...
        CancellationToken ct)
    {
        ...
        return Results.Ok(new
        {
            ...
            stepsDone = story.Steps.Count(s => s.Status == StepStatus.Completed),
            stepsTotal = story.Steps.Count,
            durationMinutes = Math.Round(((story.CompletedAt ?? DateTimeOffset.UtcNow) - story.CreatedAt).TotalMinutes, 1),
